...
MIN_GAIN = 0.01    # Минимальное значение gain (spectral floor)
                   # 0.01 = -40 dB, агрессивное подавление шума с минимальными артефактами
                   # 0.1 = -20 dB оставляет слишком много фонового шума!
                   # Диапазон: 0.01 (агрессивное, чистый звук) - 0.05 (консервативное)
...
NOISE_ESTIMATE_ADAPT_ALPHA = 0.995  # Коэффициент сглаживания для адаптивного обновления
                                     # 0.995 = медленная адаптация, безопаснее для птиц (не включит в модель шума)
                                     # 0.99 = слишком быстро, может включить фоновые звуки в модель шума
