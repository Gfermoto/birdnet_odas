...
# Параметры Log-MMSE (оптимизированы на основе Ephraim & Malah, 1985 и практик для записи птиц)
ALPHA = 0.70       # Decision-Directed smoothing factor
                   # 0.70 обеспечивает баланс: эффективное подавление нестационарного шума (ветер)
                   # при минимизации музыкальных артефактов и сохранении естественности звука
                   # Диапазон: 0.50-0.70 для минимальных артефактов, 0.85-0.95 для максимального подавления шума
NOISE_FRAMES = 15  # Количество frames для начальной оценки шума
                   # 15 frames (~0.5 сек при 16kHz) обеспечивает точную оценку шума
...
# Spectral floor для предотвращения музыкальных артефактов
MIN_GAIN = 0.1     # Минимальное значение gain (spectral floor)
                   # 0.1 = -20 dB, предотвращает резкие переходы gain и музыкальные "звоны"
                   # Стандартное значение для речи и вокализаций
                   # Диапазон: 0.05 (агрессивное) - 0.15 (консервативное, минимум артефактов)
...
NOISE_ESTIMATE_ADAPT_ALPHA = 0.99   # Коэффициент сглаживания для адаптивного обновления
                                     # 0.99 = быстрее адаптация к переменным условиям (~100 frames = 3 сек)
                                     # 0.995 = медленнее, безопаснее для птиц (не включит в модель шума)
NOISE_ESTIMATE_ADAPT_BETA = 0.01    # 1 - NOISE_ESTIMATE_ADAPT_ALPHA
...
def log_mmse_gain(xi, gamma):
    ...
    G : ndarray, shape (N_FFT,)
        Gain mask в диапазоне [MIN_GAIN, 1]

    Notes:
    -----
    - E₁(ν) - exponential integral первого порядка (scipy.special.exp1)
    - Численная стабильность: ν >= 1e-10
    - Gain ограничен к [MIN_GAIN, 1]. MIN_GAIN (spectral floor) предотвращает
      резкие переходы gain между frames и музыкальные артефакты
    """
    # ν = (ξ/(1+ξ)) × γ
    nu = (xi / (1 + xi)) * gamma
    ...
    # Clip к [MIN_GAIN, 1] для предотвращения музыкальных артефактов
    # MIN_GAIN (spectral floor) предотвращает полное подавление частот
    # и резкие переходы gain, которые создают тональные "звоны"
    G = np.clip(G, MIN_GAIN, 1)
    ...
    try:
        while True:
            ...
            while len(input_buffer) >= FRAME_SIZE * 6:
                ...
                # Адаптивная оценка шума (Ephraim & Malah, 1985)
                # Первые NOISE_FRAMES: накопление для начальной оценки
                # После: медленное адаптивное обновление для отслеживания изменений шума
                ...
                # Log-MMSE gain (классический алгоритм без модификаций)
                # Gain автоматически адаптируется к SNR: высокий SNR -> gain близок к 1,
                # низкий SNR -> gain уменьшается для подавления шума
                G = log_mmse_gain(xi_priori, gamma_post)
                
                # Применить gain к спектру
                S_enhanced = G * Y

                # Сохранить для следующего frame (используем оригинальный G для Decision-Directed)
                prev_gain = G
        ...
        if len(input_buffer) > 0:
            ...
            if noise_psd is not None:
                ...
                # Log-MMSE gain (классический алгоритм)
                G = log_mmse_gain(xi_priori, gamma_post)
