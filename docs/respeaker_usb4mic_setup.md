...
- `GAMMA_NS_SR`: 1.0 → **2.4** - усиленное шумоподавление для стационарного шума, небольшое смягчение для уменьшения артефактов (диапазон: 0.0-3.0)
- `GAMMA_NN_SR`: **1.1** - не изменяется (ограничение firmware, диапазон: 0.0-3.0)
- `MIN_NS_SR`: 0.2 → **0.15** - глубокое подавление стационарного шума с балансом для минимизации артефактов (меньше = больше подавление)
- `MIN_NN_SR`: 0.3 → **0.15** - глубокое подавление нестационарного шума (ветер) с балансом для минимизации артефактов (меньше = больше подавление)
